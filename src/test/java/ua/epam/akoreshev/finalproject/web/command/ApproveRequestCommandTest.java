...
class ApproveRequestCommandTest {
    ...
    public void setUp() {
        ...
        command = new ApproveRequestCommand(requestService);
    ...
    /**
     * @see ApproveRequestCommand#execute(HttpServletRequest, HttpServletResponse)
     */
    @Test
    void testExecuteShouldReturnUrlAndPutToSessionSuccessMessage()
            throws ServiceException, RequestException, CommandException {
        when(requestService.approveRequest(anyLong(), anyLong(), anyLong(), anyLong())).thenReturn(true);
    ...
    void testExecuteShouldReturnUrlAndPutToSessionErrorMessage()
            throws ServiceException, RequestException, CommandException {
        when(requestService.approveRequest(anyLong(), anyLong(), anyLong(), anyLong())).thenReturn(false);
        assertEquals(req.getHeader("referer"), command.execute(req, resp));
    ...
    void testExecuteShouldReturnUrlAndPutToSessionErrorMessageWhenServiceThrowRequestException()
            throws ServiceException, RequestException, CommandException {
        when(requestService.approveRequest(anyLong(), anyLong(), anyLong(), anyLong()))
                .thenThrow(new RequestException("Cannot add request"));
        assertEquals(req.getHeader("referer"), command.execute(req, resp));
        verify(session).setAttribute("isErrorMessage", true);
        verify(session).setAttribute(eq("message"), any());
    }
